...
object SafScanner {
    ...
    fun scan(context: Context, treeUri: Uri): JSONObject {
        ...
        val files = JSONArray()
        val stats = JSONObject().put("files", 0).put("videos", 0).put("directories", 0).put("errors", JSONArray())
        val pending = ArrayDeque<Pair<DocumentFile, String>>()
        pending.addLast(root to "")
        while (pending.isNotEmpty()) {
            val (directory, currentPath) = pending.removeLast()
            try {
                ...
                directory.listFiles().forEach { file ->
                    ...
                    if (file.isDirectory) {
                        pending.addLast(file to relativePath)
                    } else {
                ...
                stats.getJSONArray("errors").put("Não foi possível ler a subpasta: " + exception.message)
            }
        }
        val partial = stats.getJSONArray("errors").length() > 0
        return JSONObject().put("treeUri", treeUri.toString()).put("name", root.name ?: treeUri.toString())
            .put("documents", files).put("stats", stats).put("partial", partial)
    }
}

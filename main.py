...
async def main(page: ft.Page):
    ...
    google_account = GoogleAccountController(store)
    ...
    def render_current():
        if navigation.current == "home":
            ...
            show(SettingsView.build(page,store,library,navigate_back,on_catalog_changed,add_folder,refresh_library,login,logout,google_account.account(),google_account.state,
    ...
    async def login(_=None):
        if not google_account.begin():
            return
        if bridge.available:
            if not GOOGLE_WEB_CLIENT_ID:
                google_account.configuration_required(); refresh_settings_if_active()
            ...
            google_account.awaiting_google(); refresh_settings_if_active()
            try:
                bridge.sign_in(GOOGLE_WEB_CLIENT_ID)
            except Exception:
                google_account.fail(); refresh_settings_if_active()
                raise
            return
    ...
    def logout(_=None):
        google_account.state = 'disconnecting'; refresh_settings_if_active()
        try:
            google_account.logout()
    ...
    async def login_done(e):
        ...
        user=page.auth.user
        if user:
            google_account.complete({'id':str(user.id),'name':str(user.get('name','')),'email':str(user.get('email','')),'picture':str(user.get('picture',''))})
        refresh_settings_if_active()
        ...
        while True:
            for event in bridge.drain():
                ...
                if event_type == 'saf_scan':
                    ...
                    google_account.awaiting_google(); refresh_settings_if_active()
                elif event_type == 'google_sign_in_success':
                    if google_account.complete(payload):
                        page.snack_bar=ft.SnackBar(ft.Text('Conta Google conectada.')); page.snack_bar.open=True; page.update()
                    else:
                        page.snack_bar=ft.SnackBar(ft.Text('O Google retornou uma conta inválida. Tente novamente.')); page.snack_bar.open=True; page.update()
                    refresh_settings_if_active()
                    ...
                    refresh_settings_if_active()
                elif event_type == 'saf_selection_confirmed':
                    # Permission is persistent before this event is emitted;
                    # scanner progress must not keep the picker button locked.
                    saf_selection.finish()
                    refresh_settings_if_active()
                    ...
                    if event_type == 'google_error': google_account.fail()
